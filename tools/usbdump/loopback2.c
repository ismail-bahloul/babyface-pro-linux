// loopback2.c — bisect the loopback OFF semantics: ON ch 0/1, then try
// progressively wider 0x0000 clears and print the IN ch0/1 level each
// time. Answers: which channels does OFF need, and is 0x0000 even right?
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static double level_ms(int ms, int ch) {
...
int main(void) {
    ...
    printf(">>> loopback bisect — IN ch0 (AN1) level per step <<<\n");
    printf("  baseline (no writes)            : %6.1f dBFS\n", level_ms(1500, 0));
    ctl(0x15, 0x0001, 0); ctl(0x15, 0x0001, 1);
    printf("  ON ch0/1                        : %6.1f dBFS\n", level_ms(1500, 0));
    ctl(0x15, 0x0000, 0); ctl(0x15, 0x0000, 1);
    printf("  OFF ch0/1                       : %6.1f dBFS\n", level_ms(1500, 0));
    ctl(0x15, 0x0000, 2); ctl(0x15, 0x0000, 3);
    printf("  OFF ch0-3                       : %6.1f dBFS\n", level_ms(1500, 0));
    ctl(0x15, 0x0000, 4); ctl(0x15, 0x0000, 5);
    printf("  OFF ch0-5                       : %6.1f dBFS\n", level_ms(1500, 0));
    ctl(0x15, 0x0000, 6); ctl(0x15, 0x0000, 7);
    printf("  OFF ch0-7                       : %6.1f dBFS\n", level_ms(1500, 0));
    for (int i = 8; i < 30; i++) ctl(0x15, 0x0000, i);
    printf("  OFF all 30                      : %6.1f dBFS\n", level_ms(1500, 0));
