// an2probe.c — why is AN2 silent? Sets up 48V AN2 + gain 35, reads the
// 0x17 readback (bit1 = 48V AN2) and measures IN ch1 (the AN2 recording
// channel) so we can tell a preamp problem from a routing problem.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int read17(unsigned char *st) {
    struct usbdevfs_ctrltransfer c; memset(&c, 0, sizeof(c));
    unsigned char buf[4] = {0};
    c.bRequestType = 0xC0; c.bRequest = 0x17; c.wValue = 0; c.wIndex = 0;
    c.wLength = 4; c.data = buf; c.timeout = 1000;
    int r = ioctl(fd, USBDEVFS_CONTROL, &c);
    memcpy(st, buf, 4);
    return r;
}
...
static void measure(int ms, const char *label) {
    ...
    printf("  %-30s ch0(AN1): %6.1f  ch1(AN2): %6.1f dBFS\n",
...
int main(void) {
    ...
    unsigned char st[4];
    read17(st);
    printf(">>> AN2 probe — 0x17 readback BEFORE: %02X %02X %02X %02X (bit1 = 48V AN2) <<<\n",
           st[0], st[1], st[2], st[3]);
    ctl(0x17, 0x000E, 0x003F); ctl(0x21, 0x0000, 0x0000);  /* 48V AN2 */
    ctl(0x1A, (11 & 0x1F) | 0x20, 0x0001);                  /* gain ~35 dB AN2 */
    usleep(300000);
    read17(st);
    printf(">>> AN2 probe — 0x17 readback AFTER:  %02X %02X %02X %02X (expect bit1 = 0x02) <<<\n",
           st[0], st[1], st[2], st[3]);
    measure(4000, "TALK/RUB the mic on AN2");
    printf(">>> DONE — if ch1 stays ~-100 dBFS the preamp/48V is the problem <<<\n");
