// PAD test on AN2 (mic plugged there): stream + 48V AN2 + routing,
// then toggle PAD (bit 5 = 0x10 << 1) with RMS measurement. The relay
// click + a ~20 dB level drop confirm the PAD engages.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
...
int main(void) {
    ...
    /* 48V AN2 (bit 1), gain raw 17, routing AN2 -> AN1/2, master 0x4000 */
    ctl(0x17, 0x000E, 0x003F); ctl(0x21, 0x0000, 0x0000);
    ctl(0x1A, (17 & 0x1F) | 0x20, 0x0001);
    ctl(0x12, 0x4000, 0x0035 | 0xC000);
    ctl(0x12, 0x4000, 0x004F | 0xC000);
    ctl(0x1A, 0x00F3, 0x0004); ctl(0x1A, 0x00F3, 0x0005);
    ctl(0x12, 0x4000, 0x03E0 | 0xC000);
    ctl(0x12, 0x4000, 0x03E1 | 0xC000);

    printf("streaming, 48V AN2, gain 17. KEEP QUIET (noise-floor measurement)!\n");
    printf("  PAD off:   RMS=%9.1f\n", rms_ch1(2000));
    ctl(0x17, 0x000E | 0x20, 0x003F);   /* + PAD mic2 (bit 5) */
    ctl(0x21, 0x0000, 0x0000);
    printf(">>> PAD ON (0x2E) — LISTEN FOR THE CLICK <<<\n");
    printf("  PAD on:    RMS=%9.1f\n", rms_ch1(2000));
    ctl(0x17, 0x000E, 0x003F);          /* PAD off */
    ctl(0x21, 0x0000, 0x0000);
    printf(">>> PAD OFF (0x0E) — LISTEN FOR THE CLICK <<<\n");
