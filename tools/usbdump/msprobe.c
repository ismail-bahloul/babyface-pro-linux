// msprobe.c — OBJECTIVE MS-proc validation through the loopback: the
// loopback feeds the AN1/2 OUTPUT back into the IN stream, so the IN
// level = the monitor level. Chain: mic (AN1) -> AN 1>2 -> AN2 ->
// AN2 crosspoints (0x1000, -6 dB) -> output -> loopback -> IN ch0/1.
// MS proc ON writes 0x0000 to the AN2 crosspoints -> the loopback level
// should DROP. Rub/talk into the mic continuously while it runs.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void measure(int ms, const char *label) {
    ...
    printf("  %-40s ch0(AN1): %6.1f  ch1(AN2): %6.1f dBFS\n",
...
int main(void) {
    ...
    /* AN2 -> AN1/2 out at -6 dB (stable loop gain) */
    ctl(0x12, 0x1000, 0x0035 | 0xC000);
    ctl(0x12, 0x1000, 0x004F | 0xC000);
    ...
    printf(">>> MS proc probe — RUB/TALK into the mic continuously <<<\n");
    measure(2500, "AN1>2 ON (expect signal via loop)");
    ctl(0x12, 0x0000, 0x0001); ctl(0x12, 0x0000, 0x0035);
    ctl(0x12, 0x0000, 0x001B); ctl(0x12, 0x0000, 0x004F);
    measure(2500, "MS proc ON  (expect DROP)");
    ctl(0x12, 0x1000, 0x0001); ctl(0x12, 0x1000, 0x0035);
    ctl(0x12, 0x1000, 0x001B); ctl(0x12, 0x1000, 0x004F);
    measure(2500, "MS proc OFF (expect back)");
    printf(">>> DONE <<<\n");
