// Meter response vs preamp gain: same stream, gain raw 0 vs 17.
// Settles whether the GUI's flat VU is just the "gain starts at 0" default.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
...
static void pump(int ms) {
        ...
        if (ioctl(fd, USBDEVFS_REAPURBNDELAY, &done) == 0) {
            if (done->endpoint == 0x82) {
                ...
                for (int f = 0; f < LEN / 56; f++) {
                    unsigned int w = (unsigned int)b[f*56+1] | ((unsigned int)b[f*56+2]<<8) | ((unsigned int)b[f*56+3]<<16);
                    int s = (int)(w << 8) >> 8;   /* sign-extend 24-bit ch0 = AN1 */
                    long long m = s < 0 ? -(long long)s : (long long)s;
                    if (m > peak) peak = m;
                }
            }
    ...
    double dbfs = 20.0 * log10((double)peak / 8388608.0);
    printf("  [%5.1fs] AN1 peak=%8lld  %7.1f dBFS\n", (us() - t0) / 1e6, peak, dbfs);
}
int main(void) {
    fd = open(DEV, O_RDWR);
    ...
    t0 = us();
    printf(">>> PHASE 1: GAIN 0 (the GUI's start state) — SPEAK into the mic (5s) <<<\n");
    ctl(0x1A, 0x20, 0x0000);   /* gain raw 0 */
    pump(5000);
    printf(">>> PHASE 2: GAIN 17 (55 dB) — SPEAK into the mic (5s) <<<\n");
    ctl(0x1A, 17 | 0x20, 0x0000);   /* gain raw 17 */
    pump(5000);
