// Which bit is AN2's PAD? Toggle bit 4 (0x1E) then bit 5 (0x2E) while
// the user speaks on AN2 — the one that drops the level is AN2's pad.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
...
int main(void) {
    ...
    printf(">>> SPEAK CONTINUOUSLY. base 0x0E (3s) <<<\n");
    pump(3);
    ctl(0x17, 0x000E | 0x10, 0x003F); ctl(0x21, 0x0000, 0x0000);  /* bit 4 */
    printf(">>> BIT 4 (0x1E) — does the level DROP? (4s) <<<\n");
    pump(4);
    ctl(0x17, 0x000E, 0x003F); ctl(0x21, 0x0000, 0x0000);
    printf(">>> back to 0x0E (3s) <<<\n");
    pump(3);
    ctl(0x17, 0x000E | 0x20, 0x003F); ctl(0x21, 0x0000, 0x0000);  /* bit 5 */
    printf(">>> BIT 5 (0x2E) — does the level DROP? (4s) <<<\n");
    pump(4);
    ctl(0x17, 0x000E, 0x003F); ctl(0x21, 0x0000, 0x0000);
    printf(">>> back to 0x0E (3s) <<<\n");
    pump(3);
