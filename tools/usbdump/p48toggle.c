// Confirm the 48V mapping: 0x000D = ON, 0x000C = OFF, toggling while
// the stream runs. The user watches the P48 LED.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
...
static void pump(long ms, int mark) {
    long long t0 = us();
    while (us() - t0 < ms * 1000) {
    ...
    v17(mark ? "state after" : "state after");
}
int main(void) {
    fd = open(DEV, O_RDWR);
    ...
    printf(">>> 48V ON (0x000D) — watch P48 (3s) <<<\n");
    ctl(0x17, 0x000D, 0x003F); ctl(0x21, 0x0000, 0x0000);
    pump(3000, 1);

    printf(">>> 48V OFF (0x000C) — watch P48 (3s) <<<\n");
    ctl(0x17, 0x000C, 0x003F); ctl(0x21, 0x0000, 0x0000);
    pump(3000, 0);

    printf(">>> 48V ON again (0x000D) — watch P48 (3s) <<<\n");
    ctl(0x17, 0x000D, 0x003F); ctl(0x21, 0x0000, 0x0000);
    pump(3000, 1);

    ctl(0x17, 0x000C, 0x003F); ctl(0x21, 0x0000, 0x0000);  /* leave it OFF */
    close(fd);
    return 0;
}
