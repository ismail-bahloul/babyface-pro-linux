// lr_test2.c — how does the device build the Phones (PH3/4) L/R?
// Same RE session as lr_test.c, but writes the PB1 crosspoints into the
// Phones block ONE SIDE AT A TIME and reads words 2/3:
//   test A: L-reg 12 (0x0040) = 0dB only  -> what lands on w2/w3?
//   test B: R-reg 13 (0x005B) = 0dB only  -> what lands on w2/w3?
//   test C: both (the normal stereo pair).
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void report(const char *tag) {
    printf("  %-14s w2: 440=%6.1f 880=%6.1f | w3: 440=%6.1f 880=%6.1f\n",
           tag, level_word(2, 440, 800), level_word(2, 880, 800),
                level_word(3, 440, 800), level_word(3, 880, 800));
}
int main(void) {
    fd = open(DEV, O_RDWR);
    int iface = 5; ioctl(fd, USBDEVFS_CLAIMINTERFACE, &iface);
    struct usbdevfs_setinterface si = {5, 1}; ioctl(fd, USBDEVFS_SETINTERFACE, &si);
    printf(">>> Phones block (PH3/4) — PB1 crosspoints, one side at a time (full re-init each) <<<\n");

    init_session();
    ctl(0x15, 0x0001, 2); ctl(0x15, 0x0001, 3);
    ctl(0x12, 0x16A0, 0x0040 | 0xC000);   /* L-reg 12 (PB1 L) only */
    report("L-reg only");

    init_session();
    ctl(0x15, 0x0001, 2); ctl(0x15, 0x0001, 3);
    ctl(0x12, 0x16A0, 0x005B | 0xC000);   /* R-reg 13 (PB1 R) only */
    report("R-reg only");
