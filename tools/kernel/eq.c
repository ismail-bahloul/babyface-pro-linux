...
// Hardware DSP EQ (3-band + low cut) on the Babyface Pro FS.
//
// The EQ runs on the device DSP; the host uploads the coefficients as
// 64-byte BULK OUT blocks on ep 0x0A (interface 1).  The stored-word
// format and the RBJ fit were decoded from Windows USBPcap captures
// (cap_eq3-9.pcap — see tools/usbdump/eq_biquad.md) and ported from
// the userspace reference (tuxmix-usb/src/protocol.rs), which
// reproduces the captured words to ~1 LSB.
//
// Block layout (64 bytes):
//   0x00 : ch(0/1) slope(2^n-1|0) ch(0/1) 0x80(EQ active)
//   0x04 : slot 1 (Low)  c0 c1 c2 c3   (4 x i32, value x 2^27)
//   0x14 : slot 2 (Mid)  c0 c1 c2 c3
//   0x24 : slot 3 (High) c0 c1 c2 c3
//   0x34 : c4 SHARED by the 3 slots (0x08000000 = 0 dB / identity)
//   0x38 : low-cut freq word (0x04000000 = off)
//
// Storage = RBJ cookbook biquad, normalized numerator/denominator:
//   H(z) = c4.(1 + c2.z^-1 + c3.z^-2) / (1 + c0.z^-1 + c1.z^-2)
// with c0=a1' c1=a2' c2=b1'/b0' c3=b2'/b0' c4=b0' (each x 2^27).
//
// The kernel does not use the FPU, so the math is fixed-point (Q27,
// s64 intermediates): CORDIC for sin/cos, a degree-8 exp series for
// the gain factor.  eq_selftest.c checks the words against the
// double-precision reference (worst case ~0.5 LSB across the sweep).
//
// NOTE (shared c4 @ 0x34): the device keeps ONE scale word for the
// three band slots — captured 2-band states carry a single c4 (the
// last-written band's).  Bands with gain != 0 dB are therefore not
// independently scaled; RME's own TotalMix behaves the same way.
...
/* Recompute + re-upload all four strips (rate change, probe). */
